...
function Login() {
    let {body,logInform,mainIcon,Title,link,linkDiv,registerDiv,NoAk,languegeDiv,ActiveLanguege,DeActiveLanguege}=style
        ...
        <div className={body}>
            <div className={logInform}>
                    ...
                    <div className={linkDiv}>
                        <Link to="*" ><p className={link}>{Languege?"Я забыл пароль":"I forgot password"}</p></Link>
                        <Link to="/Home" ><p className={link}>{Languege?"Войти как тренер":"Login as a coach"}</p></Link>
                    </div>
                    <div className={registerDiv}>
                        <h4 className={NoAk}>{Languege?"Нет аккаунта?":"Don't have an account?"}</h4>
                        <Link to="*" ><p className={link}>{Languege?"Зарегистрироваться":"Register"}</p></Link>
                    </div>
